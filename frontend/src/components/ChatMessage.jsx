...
const ChatMessage = ({ text, timestamp, senderName, avatarUrl, isSender }) => {
  const avatarFallback = 'https://cdn.pixabay.com/photo/2023/02/18/11/00/icon-7797704_1280.png';
  ...
  return (
    <div className={`flex w-full gap-3 mb-6 items-end  ${isSender ? 'flex-row-reverse' : 'flex-row'}`}>
      ...
      {/* Message body */}
      <div className={`flex flex-col min-w-0 max-w-[85%] ${isSender ? 'items-end' : 'items-start'}`}>
        ...
        {/* Bubble */}
        <div
          className={`relative px-4 py-3 text-[14.5px] leading-relaxed shadow-xl backdrop-blur-md select-none wrap-break-words overflow-hidden ${
            isSender 
          ...
          <p className="font-normal whitespace-pre-wrap max-w-60  md:max-w-100  wrap-break-words">{text}</p>
